...
class ActionWrapper(ActionHandler):
	...
	def set_appointment_preferences(self, appointment_preferences):
		...
		Parameters:
			appointment_preferences (ActionResponse) : An instance of ActionResponse
		"""

		try:
			from zohocrmsdk.src.com.zoho.crm.api.appointment_preference.action_response import ActionResponse
		except Exception:
			from .action_response import ActionResponse

		if appointment_preferences is not None and not isinstance(appointment_preferences, ActionResponse):
			raise SDKException(Constants.DATA_TYPE_ERROR, 'KEY: appointment_preferences EXPECTED TYPE: ActionResponse', None, None)
