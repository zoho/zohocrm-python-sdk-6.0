...
class Feature(object):
	...
	def get_parent_feature(self):
		"""
		The method to get the parent_feature

		Returns:
			Feature: An instance of Feature
		"""

		return self.__parent_feature

	def set_parent_feature(self, parent_feature):
		"""
		The method to set the value to parent_feature

		Parameters:
			parent_feature (Feature) : An instance of Feature
		"""

		try:
			from zohocrmsdk.src.com.zoho.crm.api.features.feature import Feature
		except Exception:
			from .feature import Feature

		if parent_feature is not None and not isinstance(parent_feature, Feature):
			raise SDKException(Constants.DATA_TYPE_ERROR, 'KEY: parent_feature EXPECTED TYPE: Feature', None, None)
		
		self.__parent_feature = parent_feature
		self.__key_modified['parent_feature'] = 1
	...
	def set_components(self, components):
		...
		Parameters:
			components (list) : An instance of list
		"""

		if components is not None and not isinstance(components, list):
			raise SDKException(Constants.DATA_TYPE_ERROR, 'KEY: components EXPECTED TYPE: list', None, None)
		
		self.__components = components
		self.__key_modified['components'] = 1
