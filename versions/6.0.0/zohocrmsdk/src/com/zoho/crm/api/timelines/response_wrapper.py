...
class ResponseWrapper(ResponseHandler):
	...
	def set_timeline(self, timeline):
		...
		Parameters:
			timeline (list) : An instance of list
		"""

		if timeline is not None and not isinstance(timeline, list):
			raise SDKException(Constants.DATA_TYPE_ERROR, 'KEY: timeline EXPECTED TYPE: list', None, None)
		
		self.__timeline = timeline
		self.__key_modified['__timeline'] = 1
	...
	def set_info(self, info):
		...
		try:
			from zohocrmsdk.src.com.zoho.crm.api.timelines.info import Info
		except Exception:
