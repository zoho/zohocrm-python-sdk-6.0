...
class HistoryWrapper(ResponseHandler):
	def __init__(self):
	...
	def set_history(self, history):
		...
		Parameters:
			history (list) : An instance of list
		"""

		if history is not None and not isinstance(history, list):
			raise SDKException(Constants.DATA_TYPE_ERROR, 'KEY: history EXPECTED TYPE: list', None, None)
		
		self.__history = history
		self.__key_modified['history'] = 1
	...
	def set_info(self, info):
		...
		try:
			from zohocrmsdk.src.com.zoho.crm.api.backup.info import Info
		except Exception:
